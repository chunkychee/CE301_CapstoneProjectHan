...
export function Policy8(){
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [policy8Data, setpolicy8Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:8,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy8Data.NRIC !== null && policy8Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy8Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy8Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy8Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy8Data.errmsgCantEmpty ? <span className='text-red-500'> {policy8Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy8Data.userAlrdSignUp ? <span className='text-red-500'> {policy8Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
