...
export function Policy3(){
  ...
  const [policy3Data, setpolicy3Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:3,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy3Data.NRIC !== null && policy3Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy3Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy3Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy3Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy3Data.errmsgCantEmpty ? <span className='text-red-500'> {policy3Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy3Data.userAlrdSignUp ? <span className='text-red-500'> {policy3Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
