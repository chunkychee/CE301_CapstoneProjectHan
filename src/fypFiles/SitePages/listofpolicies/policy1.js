...
export function Policy1(){
  ...
  const [policy1Data, setpolicy1Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:1,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy1Data.NRIC !== null && policy1Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy1Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy1Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy1Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy1Data.errmsgCantEmpty ? <span className='text-red-500'> {policy1Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy1Data.userAlrdSignUp ? <span className='text-red-500'> {policy1Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
