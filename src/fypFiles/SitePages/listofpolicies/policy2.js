...
export function Policy2(){
  ...
  const [policy2Data, setpolicy2Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:2,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy2Data.NRIC !== null && policy2Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy2Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy2Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy2Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy2Data.errmsgCantEmpty ? <span className='text-red-500'> {policy2Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy2Data.userAlrdSignUp ? <span className='text-red-500'> {policy2Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
