...
export function Policy7(){
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [policy7Data, setpolicy7Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:7,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy7Data.NRIC !== null && policy7Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy7Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy7Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy7Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy7Data.errmsgCantEmpty ? <span className='text-red-500'> {policy7Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy7Data.userAlrdSignUp ? <span className='text-red-500'> {policy7Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
