...
export function Policy11(){
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [policy11Data, setpolicy11Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:11,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy11Data.NRIC !== null && policy11Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy11Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy11Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy11Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy11Data.errmsgCantEmpty ? <span className='text-red-500'> {policy11Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy11Data.userAlrdSignUp ? <span className='text-red-500'> {policy11Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
