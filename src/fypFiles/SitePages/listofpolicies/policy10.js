...
export function Policy10(){
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [policy10Data, setpolicy10Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:10,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy10Data.NRIC !== null && policy10Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy10Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy10Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy10Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy10Data.errmsgCantEmpty ? <span className='text-red-500'> {policy10Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy10Data.userAlrdSignUp ? <span className='text-red-500'> {policy10Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
