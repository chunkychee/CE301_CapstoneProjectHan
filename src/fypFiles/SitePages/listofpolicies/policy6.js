...
export function Policy6(){
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [policy6Data, setpolicy6Data] = useState({
    sessionEmail:"",
    NRIC:"",
    policyid:6,
    policyActive:"Active",
  ...
  const paymentPage = (e) => {
    e.preventDefault();
    if (policy6Data.NRIC !== null && policy6Data.NRIC !== "") {
        ...
        const updatedPolicyData = {
         ...
         fetch('http://localhost:3004/checkout-session', {
          ...
          body: JSON.stringify({
            ...
            policy1details: [{ id: policy6Data.policyid, name: policyNames.policyname, price: policyNames.pricemonthly, quantity: 1}]
          }),
        })
        .then(async (res) => {
               ...
               {showNRICBox && (
                <div className="relative bg-white p-4 rounded shadow-md">
                  ...
                  <p className="text-center underline">Input your NRIC number for policy registration</p>
                    ...
                    {policy6Data.errmsgMax9 ? <span className='text-sm underline text-black'> {policy6Data.errmsgMax9}</span> : ""}<br/>
                     <input
                      type="text"
                      name="NRIC"
                      onChange={NRICinput}
                    ...
                    />      
                    <div className="mb-1"> 
                    {policy6Data.errmsgCantEmpty ? <span className='text-red-500'> {policy6Data.errmsgCantEmpty}</span> : ""}<br/>                     
                    {policy6Data.userAlrdSignUp ? <span className='text-red-500'> {policy6Data.userAlrdSignUp}</span> : ""}<br/>                     
                    </div>
                    <div>
                      {submitSuccess ?  
                        <div>
